...
static long puzzle(const char strs[ARRLEN][STRLEN], const long size)
{
    long i, j, count = 0;
    for (i = 0; i < size; ++i) {
        const long len = strlen(strs[i]) / 2;
        for (j = 0; j < len; ++j) {
            char* ch = strchr(strs[i] + len, strs[i][j]);
            if (ch) {
                if (*ch >= 'a' && *ch <= 'z') {
                    count += *ch - 'a' + 1;
                } 
...
{
    long i, j, found, count = 0;
    for (i = 0; i < size; i += 3) {
        found = 0;
        for (j = 0; strs[i][j]; ++j) {
            if (strchr(strs[i + 1], strs[i][j]) && strchr(strs[i + 2], strs[i][j])) {
                ++found;
                break;
            }
        }
